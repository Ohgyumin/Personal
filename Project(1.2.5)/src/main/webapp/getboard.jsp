...
<body class="sub_page">
	...
	<section class="about_section layout_padding mt-5">
		<div class="container">
			<div class="row">
			<img alt="" src="${data.files}" style="width:50%">
			<br>
			<form action="delDraw.do?dtitle=${data.dtitle}" method="post" style="width: 50%;">
			<div class="col-md-7">
				<input type="hidden" name="files" value="${data.files}">
				</div>
				<div>
					ID : ${data.id}
					<br>
					Title : ${data.dtitle}
					<br>
					Content : ${data.dcontent}
					<br>
					Date : ${data.ddate}
				</div>
				<br>
				<c:if test="${data.id == id }">
				<input type="submit" value="Delete" style="background: #FFFFFF;">
				</c:if>
			</form>
			</div>
		</div>
